	...
	itype = input_atoms[ii].atype;
        for(int ix=min_range; ix<=max_range; ix++){
            for(int iy=min_range; iy<=max_range; iy++){
			    ...
			    if((itype == 1 && jtype == 2) || (itype == 2 && jtype == 1) ||
			       (itype == 3 && jtype == 2) || (itype == 2 && jtype == 3)){
...
int read_input(char *filename,a_systeminfo *mdatom_info,a_coodrinates **retval){
    ...
    while(fgets(buf,sizeof(buf),fp)){
        ...
        count++;
        sscanf(buf,"%d %f %f %f %d %d",&(input_atoms[count].atype),&(input_atoms[count].loc[0]),
	...
	else if (input_atoms[count].atype == 2) {strcpy(input_atoms[count].aname,"S");}
	else {strcpy(input_atoms[count].aname,"W");}
    }
    *retval = input_atoms;
